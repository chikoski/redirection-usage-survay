...
const write = (filename, content) => new Promise((resolve, reject) => {
  fs.writeFile(filename, content, error => {
    ...
    return resolve(content.length);
  });
});

const writeCSV = (filename, header, rows) => {
  const buf = header + "\n" + rows;
  return write(`${OUT_DIR}/${filename}`, buf);
};

const main = function (filename, filter) {
  ...
  run(chrome, scrapers, { header: null, rows: [] }).then(table => {
    chrome.close();
    const header = filter(table.header).map(i => escape(formatDate(i))).join(",");
    const rows = table.rows.map(row => filter(row).map(escape).join(",")).join("\n");
...
const switches = [
  ...
  ["-l", "--latest", "Filter latest month record"],
  ["-a", "--all", "Filter latest month record"],
];

const filename = () => {
  const date = new Date();
  return `${date.getFullYear()}_${date.getMonth() + 1}.csv`;
};

const parser = new optparse.OptionParser(switches);
parser.on("help", () => log(`${process.argv0}[-hla]`));
parser.on("latest", () => main(filename(), i => [i[i.length - 1]]));
parser.on("*", () => main("all.csv", i => i));
